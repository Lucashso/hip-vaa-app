// Avulso público — form de drop-in (aula avulsa), chama edge create-pending-drop-in.
// Rota: /avulso ou /:slug/avulso
...
import { useCreatePendingDropIn } from "@/hooks/useDropInSignup";
...
export default function PublicAvulso() {
  const navigate = useNavigate();
  const { slug } = useParams<{ slug?: string }>();
  const { data: tenant, isLoading: tenantLoading } = useTenantBySlug(slug);
  const createDropIn = useCreatePendingDropIn();
  const cepLookup = useCepLookup();

  const [form, setForm] = useState<FormState>(INITIAL);
  const [errors, setErrors] = useState<Partial<Record<keyof FormState, string>>>({});

  // Mostrar o preço do tenant
  const priceCents = tenant?.drop_in_price_cents ?? 0;
  ...
  useEffect(() => {
    // Limpa erro de CEP local quando hook atualiza
    if (cepLookup.error) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await createDropIn.mutateAsync({
        tenant_id: tenant.id,
      ...
      const target = slug ? `/${slug}/avulso/pagamento` : "/avulso/pagamento";
      navigate(target, {
        state: {
          pendingDropInId: res.pending_drop_in_id,
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      >
        ...
        <h1 className="text-white font-display text-[26px] mt-3">Aula avulsa</h1>
        <p className="text-[13px] opacity-85 mt-1 leading-[1.5]">
          Sem matrícula. Você reserva uma aula em {tenant.name}, paga PIX e remarca quando quiser.
        </p>
        {priceCents > 0 && (
          <div className="mt-3 inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/15">
            <HVIcon name="wallet" size={14} color="white" />
            <span className="hv-mono text-[11px] font-bold tracking-wider">
              {formatBRL(priceCents)} · 1 aula
            </span>
          </div>
        )}
      </div>
      ...
      <div className="flex-1 overflow-auto px-5 pt-4 pb-6">
        ...
        <button
          type="button"
          disabled={createDropIn.isPending}
        ...
        >
          {createDropIn.isPending
            ? "Enviando…"
            : priceCents > 0
              ? `Reservar e pagar · ${formatBRL(priceCents)}`
              : "Continuar"}
